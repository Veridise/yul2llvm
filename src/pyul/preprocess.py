...
def prune_dead_functions(block_node: YulNode, roots: Iterable[str]):
    ...
    edges: Dict[str, List[str]] = {
        n.get_fun_name(): list() for n in block_node.children if n.is_fun_def()
    }
    ...
    block_node.children.underlying[:] = (
        n
        for n in block_node.children.underlying
        if not YulNode(n).is_fun_def() or YulNode(n).get_fun_name() in reachable
    )


def _prune_dead_functions_logging(
    blk_node: YulNode, root_nodes: Iterable[str], logger: Optional[logging.Logger]
):
    ...
    if logger:
        logger.debug(
            "Stmt count after removing unreachable functions: "
            f"{cnt_stmts_before} -> {cnt_stmts_after}"
        )


def prune_deploy_obj(contract: ContractData, logger: Optional[logging.Logger] = None):
    ...
    assert deploy_node.type == "yul_code", "Expected yul_code, got " + deploy_node.type
    ...
    ctor_calls: List[YulNode] = [
        n
        for n in blk_node.children
        if n.is_fun_call() and n.get_fun_name().startswith("constructor_")
    ]
    assert (
        len(ctor_calls) <= 1
    ...
    blk_node.children.underlying[:] = (
        n for n in blk_node.children.underlying if YulNode(n).is_fun_def()
    )
    cnt_stmts_after2 = len(blk_node.children)

    if logger:
        logger.debug(
            "Stmt count after removing non-function-def statements: "
            f"{cnt_stmts_after} -> {cnt_stmts_after2}"
        )
        logger.debug("END PASS prune_deploy_obj")


def prune_deployed_code(
    contract: ContractData, logger: Optional[logging.Logger] = None
):
    ...
    to_move = [(i, s) for i, s in enumerate(blk_node.children) if not s.is_fun_def()]
    for i, s in reversed(to_move):
        blk_node.children.underlying.pop(i)

    selector_fun_name = "_pyul_selector"
    new_selector_fun = create_yul_node(
    ...
    def walk_selector(node: YulNode, parents: List[YulNode]):
        ...
        if node.type == "yul_case":
            ...
            assert (
                len(case_body_fun_calls) == 1
            ), f"Incorrect number of function in selector case body"
            contract.metadata.external_fns[selector] = case_body_fun_calls[
                0
            ].get_fun_name()
    ...
    if logger:
        logger.debug("Stmt count change: " f"{stmt_cnt_begin} -> {stmt_cnt_end}")
        logger.debug("END PASS prune_deployed_code")


def attach_storage_layout(
    contract: ContractData, logger: Optional[logging.Logger] = None
):
        ...
        if type_dict["encoding"] == "inplace":
            if type_dict["label"].startswith("struct "):
                ...
                return ast.YulIntType(
                    pretty_name=type_dict["label"], size=int(type_dict["numberOfBytes"])
                )
    ...
    if typs is not None:
        contract.metadata.types.update(
            {name: parse_type(entry) for name, entry in typs.items()}
        )


def rewrite_storage_ops(
    contract: ContractData, logger: Optional[logging.Logger] = None
):
    ...
    * The storage layout will be used to rewrite sload/store of constants into
    ...
    svars = {(v.slot, v.offset): (v.name, v.type) for v in contract.metadata.state_vars}

    # Regexes for rewrite rules
    # Note: we don't handle the following variants:
    # - read_from_storage_offset_dynamic
    load_re = re.compile(
        "^read_from_storage(?P<issplit>_split)?_offset_(?P<offset>[0-9]+)_(?P<type>.*)$"
    )
    update_re = re.compile(
        "^update_storage_value_offset_(?P<offset>[0-9]+)?(?P<src_type>.*)_to_(?P<dest_type>)"
    )
    ...
    _prune_dead_functions_logging(
        ...
        {contract.metadata.main_ctor},
        logger,
    )


def rewrite_map(contract: ContractData, logger: Optional[logging.Logger] = None):
...
def rewrite_shift_left(contract: ContractData, logger: Optional[logging.Logger] = None):
    ...
    def _rewrite_shift_left(node: YulNode, parents: List[YulNode]):
        ...
        if match:
            ...
            node.children.underlying[:] = create_yul_node(
                node.type,
                [
...
def rewrite_call_inst(contract: ContractData, logger: Optional[logging.Logger] = None):
